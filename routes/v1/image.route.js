...
/*
 * This is a template for a custom route of image, version v1.
 *
 * The exported functions in this file will be turned into API endpoints in
 * /routes/index.ts with the path v1/image-(function name).
...
/**
 * Gets a image.
 * @param   {Number}    id  The is the id of the image you are searching for.
...
exports.get = async (body) =>  {
    ...
    let response = body.image_catalog_id ? await image.getImageListings(body.image_catalog_id) : await image.getImageRefs()
    switch (true){
        case (body.image_catalog_id !== undefined):
            response = await image.getImageListings(body.image_catalog_id)
            break;
...
/**
 * Deletes a image.
 * @param   {Number}    id  The is the id of the image you want to delete.
